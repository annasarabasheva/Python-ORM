...
def show_hard_dungeons() -> str:
    ...
    result = [f"{dungeon.name} is guarded by {dungeon.boss_name} who has {dungeon.boss_health} health points!" for dungeon in dungeons]
    return '\n'.join(result)
...
def show_workouts() -> str:
    ...
    result = [f"{w.name} from {w.workout_type} type has {w.difficulty} difficulty!" for w in calisthenics_crossfit_workouts]
    return "\n".join(result)
